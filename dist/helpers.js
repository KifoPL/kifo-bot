...
export async function setAllCommands(client) {
...
export function getAllModules(dir) {
    const modules = getAllFiles(dir);
    return modules.filter((file) => {
        return file.endsWith('.js');
    });
}
export function setEvent(client, event) {
    if (event.isOnce) {
...
export async function handleAllEvents(client) {
    console.log('=== Setting all event handlers... ===');
    const eventsPath = path.join(__dirname, './events');
    const events = getAllModules(eventsPath).filter((file) => file.endsWith('.js'));
    for (const file of events) {
        ...
        if (event.name && event.execute)
            ...
            console.error(`Event ${file} does not have a name or execute property.`);
    }
